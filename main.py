...
def start(update, context):
    update.message.reply_text('Welcome to the File Conversion Bot! Send me a file and specify the format you want to convert it to.')
...
def convert(update, context):
    ...
    if len(context.args) == 0:
        update.message.reply_text('Please specify the format you want to convert the file to.')
        return
    ...
    if desired_format not in SUPPORTED_FORMATS:
        update.message.reply_text(f'Sorry, {desired_format.upper()} format conversion is not supported.')
        return
...
def help(update, context):
    help_message = """
    Welcome to the File Conversion Bot!

    *Commands:*
    - /start: Start the bot
    - /help: Show this help message
    - /convert [format]: Convert a file to the specified format
